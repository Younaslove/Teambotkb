...
class Kaptaan:
    def __init__(self, my_app, my_token=None):
        ...
        self.kaptaanApi = "https://api.boteater.us"
    ...
    def object2Direct(self, url, ext, headers=False):
        data = {"url": url,
                "ext": ext}
        if headers == False:
            r = requests.post(self.kaptaanApi + "/line_obs?destination=local", data=data)
            return json.loads(r.text)["result"]
        if headers == True:
            r = requests.post(self.kaptaanApi + "/line_obs?destination=local", data=data, headers=self.headers)
            return json.loads(r.text)["result"]
    ...
    def object2Gdrive(self, url, ext, headers=False):
        data = {"url": url,
                "ext": ext}
        if headers == False:
            r = requests.post(self.kaptaanApi + "/line_obs?destination=gdrive", data=data)
            return json.loads(r.text)["result"]
        if headers == True:
            r = requests.post(self.kaptaanApi + "/line_obs?destination=gdrive", data=data, headers=self.headers)
            return json.loads(r.text)["result"]
